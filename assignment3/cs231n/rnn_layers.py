...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  """
    ...
    h = np.zeros((N, T, H))
    cache = {}
    for t in range(T):  # each state in a certain batch
        if t == 0:
...
def rnn_backward(dh, cache):
  ...
  """
    ...
    N, T, H = dh.shape
    x, prev_h, Wx, Wh, affine_output, next_h = cache[T - 1]
    N, D = x.shape
    dx = np.zeros((N, T, D))
...
# convert words (represented by integers) into vectors
def word_embedding_forward(x, W):
  ...
  """
    ...
    # print W[x[0, 0]]
    out = np.zeros((N, T, D))
    for n in range(N):  # like use known index to vocabulary find the word vector (choose word vector represented)
        for t in range(T):
            out[n, t, :] = W[x[n, t]]
...
def word_embedding_backward(dout, cache):
  ...
  """
    ...
    np.add.at(dW, x, dout)  # value dw at x[n,t] should add for stage gradient calculate
